...
namespace Yarn.Unity
{
    ...
    public static class CustomEffects
    {
        ...
        public static IEnumerator FadeAlpha(DialogueRunner runner, CanvasGroup canvasGroup, float from, float to, float fadeTime, CoroutineInterruptToken stopToken = null)
        {
            stopToken?.Start();
            ...
            else
            {
                ...
                canvasGroup.blocksRaycasts = runner.IsDialogueRunning;
            }
    ...
    public class CustomLineView : DialogueViewBase
    {
        ...
        public bool isRunning { get => currentStopToken.IsRunning; }
        ...
        private IEnumerator DismissLineInternal(Action onDismissalComplete)
        {
            ...
            if (useFadeEffect)
            {
                yield return StartCoroutine(CustomEffects.FadeAlpha(_runner, canvasGroup, 1, 0, fadeOutTime, currentStopToken));
        ...
        public override void InterruptLine(LocalizedLine dialogueLine, Action onInterruptLineFinished)
        {
            ...
            {
                //if (characterNameText.text != dialogueLine.CharacterName)
                //{
                //    ChangeNameText(dialogueLine.CharacterName);
                //}
            ...
            canvasGroup.interactable = true;
            canvasGroup.blocksRaycasts = _runner.IsDialogueRunning;

            //onInterruptLineFinished();
        }

        //private Coroutine nameTextCoroutine;
        //private float _nameFadeDuration = 0.3f;
        ///// <summary>
        ...
        //{
        //    float targetAlpha = characterNameText.color.a;
        //    //Fade-in first.
        //    for (float t = 0; t < _nameFadeDuration; t += Time.deltaTime)
        //    {
        //        Color targetColor = characterNameText.color;
        //        targetColor.a = 0;
        //        characterNameText.color = Color.Lerp(characterNameText.color, targetColor, Mathf.Clamp01(t / _nameFadeDuration));
        //        yield return null;
        //    }
        //    characterNameText.text = _text;
        ...
        public override void RunLine(LocalizedLine dialogueLine, Action onDialogueLineFinished)
        {
            ...
            if (_sfx)
                _source = _sfx.PlayLoopSFX("type");

            DialogueManager.instance.isNext = false;
        }
        ...
        private IEnumerator RunLineInternal(LocalizedLine dialogueLine, Action onDialogueLineFinished)
        {
            IEnumerator PresentLine()
            {
                ...
                if (useFadeEffect)
                {
                    yield return StartCoroutine(CustomEffects.FadeAlpha(_runner, canvasGroup, 0, 1, fadeInTime, currentStopToken));
            ...
            DialogueManager.instance.isNext = true;
            if (_source)
            {
                _source.loop = false;
                _source.Stop();
