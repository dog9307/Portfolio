...
namespace Michsky.UI.Dark
{
    ...
    public class UIDissolveEffect : BaseMeshEffect
    {
        ...
        [SerializeField]
        private bool _isDissolveOut = false;
        public bool isDissolveOut { get { return _isDissolveOut; } set { _isDissolveOut = value; } }
        ...
        void Update()
        {
            if (_isDissolveOut)
            {
                float loc = location + Time.unscaledDeltaTime * animationSpeed;

                if (loc == 0.99f || loc >= 0.99f)
                    OnAfterDissolveOut();
            }
        }

        public UnityEvent afterDissolveOut;
        ...
        public void OnAfterDissolveOut()
        {
            if (afterDissolveOut != null)
                afterDissolveOut.Invoke();
        }
    }
}