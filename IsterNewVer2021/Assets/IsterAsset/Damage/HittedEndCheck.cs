...
public class HittedEndCheck : StateMachineBehaviour
{
    [SerializeField]
    ...
    override public void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        if (!_rigid)
        {
            ...
            if (!_rigid)
            {
                _rigid = animator.GetComponentInChildren<Rigidbody2D>();
                if (!_rigid)
                    return;
            }
        }

        _damagable = animator.GetComponent<Damagable>();
        ...
        if (!_damagable)
        {
            ...
            if (!_damagable)
            {
                _damagable = animator.GetComponentInChildren<Damagable>();
                if (!_damagable)
                    return;
            }
        }
