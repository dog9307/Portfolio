...
public class PlayerDashAttack : Attacker
{
    [SerializeField]
    ...
    public override void AttackEnd()
    {
        ...
        AfterImageControl(false);
    }

    public override bool IsTriggered()
    {
        if (_equipment)
            _equipment.isBattle = isBattle;
        ...
        {
            if (IsCounterTriggered(HAND.LEFT))
            {
                AttackTriggered(ATTACK_TYPE.COUNTER, HAND.LEFT);
            ...
            {
                if (!_delay.isLeftDelay)
                {
                    if (!charge)
                    {
                        AttackTriggered(ATTACK_TYPE.NORMAL, HAND.LEFT);
                        return true;
                    }
                    else
                    {
        ...
        {
            if (IsCounterTriggered(HAND.RIGHT))
            {
                AttackTriggered(ATTACK_TYPE.COUNTER, HAND.RIGHT);
            ...
            if (!_skill.isSkillUsing)
            {
                if (!_delay.isRightDelay)
                {
                    if (!charge)
                    {
                        AttackTriggered(ATTACK_TYPE.NORMAL, HAND.RIGHT);
                        return true;
                    }
                    else
                    {
